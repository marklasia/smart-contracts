...
async function testMultiBuyTokens(investors, contract, args) {
  ...
  for (const investor of investors) {
    ...
    const preEthDust = await contract.unclaimedPayoutTotals(investor)
    ...
    const postEthDust = await contract.unclaimedPayoutTotals(investor)
    ...
    const expectedEthDust = investAmount
      .mul(1e18)
      .mul(initialSupply)
      .mod(fundingGoal.div(1e18))
    ...
    assert.equal(
      postEthDust.sub(preEthDust).toString(),
      expectedEthDust.toString(),
      'the dust, if any should be put in unclaimedPayoutTotals'
    )
    assert.equal(
...
async function testFallbackBuy(web3, contract, args) {
  ...
  const expectedTokenPurchaseAmount = investAmount
    ...
    .div(fundingGoal)
    .toFixed(0)
  const expectedDust = investAmount
    .mul(1e18)
    .mul(initialSupply)
    .mod(fundingGoal.div(1e18))
  ...
  const postContractTokenBalance = await contract.balanceOf(contract.address)
  const postContractEtherBalance = await getEtherBalance(contract.address)
  const postInvestorEthDust = await contract.unclaimedPayoutTotals(investor)
  ...
  assert.equal(
    postInvestorEthDust.sub(preInvestorEthDust).toString(),
    expectedDust.toString(),
    'the dust, if any should be put into unclaimedPayoutTotals'
  )
  assert.equal(
...
async function testBuyRemainingTokens(contract, accounts, args) {
  ...
  assert.equal(
    ...
    'the investor token balance should be incremented by that bought excluding overpaid amount'
  )
  assert.equal(
    postInvestorEtherBalance.toString(),
  ...
  assert.equal(
    postInvestorEthDust.sub(preInvestorEthDust).toString(),
    new BigNumber(0),
    'Ξ dust change should be 0 if last buyer'
  )
  assert.equal(
...
async function testKill(owner, investor, contract) {
  const preContractEtherBalance = await getEtherBalance(contract.address)
  const preOwnerEtherBalance = await getEtherBalance(owner)
  ...
  const postContractEtherBalance = await getEtherBalance(contract.address)
  const postOwnerEtherBalance = await getEtherBalance(owner)
  //const expectedPostOwnerEtherBalance =
